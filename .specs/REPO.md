...
>
> **Known exception (feature 006-json-everywhere).** A third deviation, extending the 004/005 precedent: the read tools' text formatters were replaced with raw-JSON passthrough (new `src/helpers/json-response.ts`; ~21 `src/tools/list/**` + `src/tools/get/**` files converted; `src/helpers/pagination-hint.ts` and `src/helpers/format-line-item.ts` deleted) — an owner-approved output-format change recorded in **ADR-0005**, shipped as minor v0.3.0. `git diff upstream/main -- src/ ':!src/http'` remains non-empty for this additional, named reason on top of features 004 and 005's.
>
> **Known exception (feature 007-response-shape).** A fourth deviation, extending the 004/005/006 precedent: the 5 report tool files (`list-trial-balance.tool.ts`, `list-profit-and-loss.tool.ts`, `list-report-balance-sheet.tool.ts`, `list-aged-receivables-by-contact.tool.ts`, `list-aged-payables-by-contact.tool.ts`) were converted from multi-block prose + pretty-printed JSON to a single `reportResponse` call, and `list-xero-accounts.handler.ts` + `list-accounts.tool.ts` gained an `activeOnly` param (default `true`) — an owner-approved deviation recorded in **ADR-0006**, shipped as minor v0.4.0. New fork-owned helper: `src/helpers/report-envelope.ts`. Deleted: `src/helpers/format-aged-report-filter.ts` (orphaned once the aged-report tools moved to `reportResponse`). `git diff upstream/main -- src/ ':!src/http'` remains non-empty for this additional, named reason on top of features 004, 005, and 006's.
>
