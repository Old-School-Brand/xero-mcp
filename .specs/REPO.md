...
>
> **Known exception (feature 005-xero-usability).** A second, deliberate deviation: general-ledger access and pagination fixes were applied *locally* to upstream-owned `src/handlers/` and `src/tools/` — an owner-approved deviation accepting the merge cost, following the feature 004 precedent. Affected files: the new `src/handlers/list-xero-account-transactions.handler.ts` and `src/tools/list/list-account-transactions.tool.ts` (GL-per-account over the Journals endpoint), the wiring edit to `src/tools/list/index.ts` (registers the new tool), and the `pageSize` bump (10 to 100) plus pagination-hint messaging in the five existing transaction handler/tool pairs — `list-xero-invoices`, `list-xero-manual-journals`, `list-xero-bank-transactions`, `list-xero-credit-notes`, `list-xero-payments` (handler + tool file each). The `offline_access` scope fix (workstream C of the same feature) stayed entirely within `src/http/` and required **no** exception. `git diff upstream/main -- src/ ':!src/http'` remains non-empty for this additional, named reason on top of feature 004's.
