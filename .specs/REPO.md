...
>
> **Reverted from 005 (workstream A — GL tool).** The `list-account-transactions` tool over Xero's Journals endpoint (new handler + tool + `index.ts` wiring) was implemented, merged, then **reverted** — Xero gates the Journals endpoint behind the **Advanced developer-app plan** (~AUD $1,445/mo) plus an initial/annual security assessment and use-case approval, which this app does not have (it's on the Starter plan), so the tool returned 403 in production. See `.specs/backlog/general-ledger-advanced-tier-access.md`. Revisit — built on the planned journal-store cache (006) — only if the org obtains Advanced access.
