...
- **005-xero-usability** (backend) — Fix two confirmed user blockers: add a `list-account-transactions` GL tool over Xero's Journals feed (`ifModifiedSince`-narrowed, offset-continuation, compact JSON), raise transaction `pageSize` 10→100, and add `offline_access` to the Entra login so sessions no longer force a full re-login. Fork-only (recorded upstream-isolation exception). Valkey response cache → 006, JSON-everywhere → 007, infra 502/OOM → 008.
- **006-json-everywhere** (backend) — Convert all read tools to raw-JSON passthrough with the `{showing, [hasMore,] rows}` list envelope (ADR-0005); shipped v0.3.0, aPIKey-redaction hotfix v0.3.1.
- **007-response-shape** (backend) — Lossless report envelope for the 5 report tools (sections/rows/total, per-row deduped attributes), global empty-value omission, minified single-block output everywhere, and `activeOnly` (default true) on `list-accounts`. From post-v0.3.1 tester feedback.
