...
| **Report envelope** | The structured JSON shape report tools return: `{report, date, updatedAt, columns, sections: [{title, rows, total}]}` with cells keyed by column title and per-row deduplicated `attributes`. | report tree (that is Xero's raw shape) |
| **Empty-value omission** | The global serialization rule dropping `""`/`null` keys while always emitting `0` and `false`, applied in the `jsonResponse` replacer. | field trimming (implies dropping populated fields) |
